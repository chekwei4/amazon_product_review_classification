...
ARG REPO_DIR="."
ARG CONDA_ENV_FILE="review-classification-conda-env.yml"
ARG CONDA_ENV_NAME="review-classification"
...
ARG PRED_MODEL_GCS_URI="gs://review-classification-artifacts/mlflow-tracking-server/$PRED_MODEL_UUID"
...
COPY $REPO_DIR review-classification
RUN mkdir $HOME_DIR/from-gcs

RUN $CONDA_BIN env create -f review-classification/$CONDA_ENV_FILE && \
    $CONDA_BIN init bash && \
    $CONDA_BIN clean -a -y && \
...
WORKDIR $HOME_DIR/review-classification
